...
Функция `test-fahr` и есть тест. Она проверяет, что никто не изменил код
...
- `cider-test-rerun-failed-tests (C-c C-t f)` повторно выполняет неудачный тест
...
то в итоге программа будет искать их в папках resources и resources_test. Во
время финальной сборки оставим только resources, чтобы не вбирать в jar ресурсы
для тестов.

Заметим, что путь resources_test выбран неудачно. Существует другой, более
...
Первый — запускают REPL с ключом `with-profile +test`, что значить добавить к
...
Первая команда создает нового пользователя БД с именем book_test. Ключ -S
...
Здесь и ниже мы полагаем, что глобальная переменная `*db*` указывает на
...
`*db*` так, как это удобно в проекте.
...
`/Users/ivan/work/book` равна нулю. Доработайте запрос так, чтобы вместо
...
`data/users.csv.gz`, пропустив его через серию функций и классов. Последний
...
по шаблону `<table>_<pk>_seq`, например, `users_id_seq`.
...
Наивный способ очистить базу — послать запрос `DELETE FROM <table>` для всех
...
*[Продолжение следует](/clj-book-tests-2)*
