...
Команда запустит процесс `ssh` без ввода с клавиатуры. Туннель будет жить до тех пор, пока вы не нажмете `Ctrl+C`. Перейдите в Emacs и подключитесь к порту 19911:
...
Для ClojureScript платформа JVM выступает в роли посредника. Ее задача — обеспечить связь между REPL и JavaScript. Связь работает поверх какого-то транспорта. Это может быть классическое REST API, веб-сокеты, TCP-соединение или что-то другое.
