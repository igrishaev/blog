...
В названии [nREPL][nrepl] буква n означает network, то есть сетевой REPL. Проект нацелен на то, чтобы обеспечить работу в REPL по сети. В отличие от терминала, nREPL обладает более сложной архитектурой; перечислим ее главные свойства.
...
Остальные параметры вы найдете в документации проектов [lein][lein-repl] и [nREPL][nrepl-options].

### Внутреннее устройство

[ring]: https://github.com/ring-clojure/ring

Архитектура nREPL состоит из нескольких частей: обработчика запроса, middleware и транспорта. В этом плане nREPL напоминает [Ring][ring]: большую роль в нем играет спецификация — соглашение о том, как устроена та или иная часть. При этом код относительно мал в сравнении со смысловой нагрузкой.
...
Проект Cider, о который мы скоро рассмотрим, предлагает [множество подобных middleware][cider-middleware]. Вместе они радикально расширяют возможности nREPL.
...
Мы не будем досконально перечислять все команды nREPL. Любопытный читатель найдет их на [странице документации][nrepl-ops]. Завершим этот раздел тем, что вызовем команду `close`. Она принимает строго один параметр — номер сессии — и освобождает ресурсы, связанные с ней:
...
Кроме импоров, в `dev` размещают служебные функции, например:
