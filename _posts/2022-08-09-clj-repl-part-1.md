...
{% include toc.html id="clojure-repl-toc" title="" %}
...
В мире Лиспа ходит понятие REPL-driven development. Это стиль разработки, когда код пишут код малыми порциями и запускают в REPL. С таким подходом сразу видно поведение программы. Легче проверить неочевидные случаи, например вызвать функцию с `nil` или обратиться к ресурсу, которого не существует.
...
По умолчанию REPL выводит краткое сообщение об ошибке. Объект исключения остается в переменной *ee. Исследуем ее:
...
Если вы запустили REPL через `lein`, вам доступны некоторые удобства. Выражение `(doc ...)` напечатает справку к указанной функции, например:
...
Функция `eval` принимает выражение и вычисляет его. В отличии от других языков, в Clojure `eval` принимает не строку, а форму, то есть примитив (число, строку), символ или коллекцию. Примитивы вычисляются сами в себя:
