...
Докер. Но на Маке он скоро будет нативный, уже есть бета-версия.
...
- При создании Докера обновить информацию о пакетах.
...
Make-переменная `$(CURDIR)` означает текушую папку. К сожалению, при пробросе
томов нельзя указать относительный путь, а хардкодить гите -- моветон.
