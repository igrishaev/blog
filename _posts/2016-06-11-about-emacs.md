...
Сказаное ниже носит общий характер. Это не мануал и не инструкция с шагами что
...
айти-инустрии и все еще остается в строю.
...
Подобно Лиспу, у Емакса есть форки и отвлетвления. Так, в девяностых годах
...
рода улучшенный Емакс. О форках я раскажу ниже.
...
большинство операций в Виме делаются быстрее. В каждом режиме клавша имеет
...
инфофрмации -- текст, Емакс хорошо подходит для интеграции с системой. Само
...
пальцы уже нельзя преучить после 20 лет Емакса. Гвидо положительно отозывлся о
...
- Для Винды. [Скачать бинарники](https://ftp.gnu.org/gnu/emacs/windows/. На
  винде осложнения в том, что Емакс нуждатся в стандартных Юникс-утилитах ps,
...
Второе -- займитесь слепой печатью. Зайдите на сайт http://www.keybr.com/
. Зарегистрируйтесь через соц.сеть, чтобы накапливалась статистика. Каждый день
утром 15 минут печатайте вслепую. Сначала будет бесить, но статистика покажет
медленное продвижение, и это классно. Вы должны как можно скорее избавиться от
стрелочек и пользоваться только центральной частью клафиатуры.

![chart](/assets/static/emacs/chart.png)

*Один из графиков моего профиля. Он показывает, что скорость набора медлено, но
растет.*
...
файлы). Я никода не выключаю ноут, а только погружаю в сон. К концу второго
...
должны понимать, что приосходит и как этим управлять. Вы должны знать назначение
...
Я терпеть не могу длинных навороченных конфигураций со могими файлами. При
...
Для емакса напасаны тысячи пакетов, некоторые из которых тянут на полноценные
приложения, исполняемые внутри Лисп-машины.

Пакетых хостятся в репозиториях, у которых следующие имена -- `GNU, Melpa,
...
Как уже упоминал выше, существует XEmacs -- альрнативная ветка Емакса. В ней
...
Проект [Эрго-Емакс](https://github.com/ancane/emacs-nav) ставит цель создать
...
Наконец, существует [Spacemacs](https://github.com/ancane/emacs-nav) -- попытка
взять лучшее из двух миров. Соединить механику Вима с мощью платформы
...
и воощбе, идея смешения мне не нравится. Она наивна по своей природе. Вот у нас
есть Руби, а есть Питон, у них достоинства и недостатки. Давайте возьмем от
каждого только достоинства и получим идеальный язык! Или давайте Мерседес с Ауди
смешаем. Или Виндуз с Маком. Это не работает.
...
- Изредка читаю онлайн-документацию во встроенном браузере EWW. Это полезно,
  когда не хочень постоянно переключаться из браузера в редактор. Делишь экран
...
- Запускаю и контроллирую процессы с помощью пакета prodigy. Был проект на
...
- Иногда делаю презентации в емаксе: пишу структурированный файл в разметке ORG,
  ...
  юнит-тестах: [исходник](https://github.com/ancane/emacs-nav),
  [результат](https://github.com/ancane/emacs-nav).
...
Тадам. И это даже не верхушка айзберга, а самая мелочь из того, что можно делать
в org-mode.

- Возвращаюсь к нашему списку. Веду GDT -- учет личных и рабочих дел. Давно хочу
  написать на эту тему в блог, по пока что отдельным абзацем.
...
воркфлоу вы задаете сами в шапке файла. Им можно выставлять теги, иерарию,
...
Я перенес все задачи в такой файл и храню его в приватном репозитории. Каждое
утро смотрю, какие задачи меня ждут и стараюсь их выполнять. Задачи можно
архивировать. Емакс вырезает их из файла и переносит в ахвивный файл с похожим
названием.

Я не могу расшарить свой файл, поскольку в нем немало приватных данных, но
...
Это бытовые дела -- позвонить, подписать, сходить в Сбербанк. Все задачи я
группирую по крупным разделам: работа, семья и дети, бизнес, блог, сторонний
проект, новинки. Почти ничего я не правлю руками, все сущности создаются
командами и хоткеями.
...
систему тебя пинать. То, что я описал, мне очень подошло.

### Ресурсы

- [Официальный туториал](https://github.com/ancane/emacs-nav)
- Подробнейшая [вики](https://github.com/ancane/emacs-nav)
- [Блог китаянки](https://github.com/ancane/emacs-nav), у которой конфиг Емакса
  длиной с Китайскую Стену
- Довольно свежая [книжка](https://github.com/ancane/emacs-nav)
- [Серия статей](https://github.com/ancane/emacs-nav) Алекса Отта
- Раздел на [Хабрахабре](https://github.com/ancane/emacs-nav)
