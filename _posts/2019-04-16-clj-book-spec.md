...
результат в вектор, где первый элемент – элемент логического пути. Такой пусть
...
структура `s/conform` будет отличаться от входных данных. Например, на месте
...
Топология выходного словаря отличается входных данных:
...
правильно снабдить код ответа понятным описанием. Скорее всего клиент запишет
...
подход из примера выше требует, чтобы чтобы для каждого такого ключа было
сообщение об ошибке. Это склоняет нас к повторам в коде.
...
грамматике `(секция, пара*)*`, где звездочка означает сколько угодно раз, в
т.ч. ничего.
...
Напишем спеку, чтобы чтобы проверить конфигурацию и вывести типы. Например,
чтобы номера портов были числами:
...
Наоборот, во время прогона тестов быстродействие нас не интересует. Мы стремимся
покрыть код как можно большим числом проверок, чтобы отловить необычные
сценарии.
...
date-range-sec справедливо правило: если первая дата больше второй, результат
будет отрицательным. Реализуйте спеку `:fn`, которая проверяет это условие. Тем
самым вы предотвратите случай, когда кто-то решит, что результат должен быть по
модулю.
