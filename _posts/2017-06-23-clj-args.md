...
arguments. The first found body is called. Then no body is found, an exception
raised.
...
Clojure dispatches a proper body very fast. It is a key feature of Clojure
...
function accepts only a couple of required parameters but the rest are put into
...
since it brings lots of noise in the code:
...
passed one to fallback to default values:
...
(defn foo [hostname port & [opt]]
  (let [{:keys [timeout
    ...
    ;; now timeout will always be equal to 5 when not passed
    ...
    ))
~~~
...
3. With the rest arguments, `partial` works like a charm. Say, we have a
   ...
   a table name, and the rest are a sequence with each add argument as a column
   name and even argument as a value:

   ~~~clojure
   (db-query :users :active false :staff true :name "Ivan")
   ...
   clause every time we query users. To prevent returning blocked ones to the
   frontend, it's better to have a special function for that:

   ~~~clojure
   (def active-users (partial db-query :users :active false))
   ~~~
