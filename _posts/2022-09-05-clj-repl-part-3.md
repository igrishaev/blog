...
Если ее выполнить, процесс будет похож на настоящую отладку. Сперва вы окажетесь в первой точке `(break 1)`. В этот момент еще не доступна ни одна локальная переменная. В точке `(break 2)` появится доступ к переменной `а`. Выйдя из нее, вы окажетесь в третьей точке, где доступны `a` и `b`. Покинув третью точку, вы получите результат 3.
