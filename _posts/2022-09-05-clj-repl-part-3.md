...
Плагин [Clojure mode][clojure-mode] расширяет Imenu в Emacs. Imenu (сокращение от Interactive menu) — это встроенный модуль для показа определений в файле. По команде `M-x imenu` откроется буфер с оглавлением — именами функций, макросов, типов — и приглашением ввода. Приведем краткую версию буфера для модуля `clojure.core`:
...
~~~
В
о втором сообщении клиент отправит команду `test` со списком из одного теста:
